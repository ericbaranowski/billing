...
<div class="form-group"><div class="btn-toolbar" role="toolbar"><button type="button" onclick="window.location='supporttickets.php'" class="btn btn-default">{$LANG.clientareabacklink}</button><button type="button" onclick="jQuery('#replycont').slideToggle()" class="btn btn-default"><i class="fa fa-reply"></i> {$LANG.supportticketsreply}</button> {if $showclosebutton} <button type="button" onclick="window.location='{$smarty.server.PHP_SELF}?tid={$tid}&amp;c={$c}&amp;closeticket=true'" class="btn btn-default pull-right">{$LANG.supportticketsclose}</button>{/if}</div></div>
...
<div class="panel panel-default">
    ...
    <div class="{if $reply.admin}admin{else}client{/if}header panel-heading">
        ...
        <h4 class="panel-title">{if $reply.admin}
            ...
            {elseif $reply.userid}
            <img src="https://www.gravatar.com/avatar/{php}$userid = $this->_tpl_vars['clientsdetails']['userid'];$result = mysql_query("SELECT email FROM tblclients WHERE id=$userid");$data = mysql_fetch_array($result);$email = $data["email"];echo md5( strtolower( trim( $email ) ) );{/php}?s=30&d=mm" class="menu-avatar" alt="">{$reply.name}
            {else}
   ...
   <div class="form-group"><div class="btn-toolbar" role="toolbar"><button type="button" onclick="window.location='supporttickets.php'" class="btn btn-default">{$LANG.clientareabacklink}</button><button type="button" onclick="jQuery('#replycont2').slideToggle()" class="btn btn-default"><i class="fa fa-reply"></i> {$LANG.supportticketsreply}</button> {if $showclosebutton} <button type="button" onclick="window.location='{$smarty.server.PHP_SELF}?tid={$tid}&amp;c={$c}&amp;closeticket=true'" class="btn btn-default pull-right">{$LANG.supportticketsclose}</button>{/if}</div></div>

...
<div data-toggle="tab" id="tab1" class="tab-content">
    ...
    <div class="row productdeets">

        <div class="col30" style="width: 25%;">
            <div class="internalpadding">
                <div class="styled_title"><h2>{$LANG.information}</h2></div>
				<div class="box">
					<p>{$LANG.clientareaproductdetailsintro}</p>
					<p><input type="button" value="{$LANG.backtoserviceslist}" class="btn btn-primary" onclick="window.location='clientarea.php?action=products'" /></p>
				</div>
			</div>
        </div>
        <div class="col70 product-details">
                ...
                <div class="clear"></div>
                {if $suspendreason}<p><h4>{$LANG.suspendreason}:</h4> <span>{$suspendreason}</span></p>{/if}
                {if $lastupdate}
                <div class="col2half">
                ...
                {/foreach}
                <div class="clear"></div>
                {if $moduleclientarea}<div class="moduleoutput">{$moduleclientarea|replace:'modulebutton':'btn btn-primary'}</div>{/if}
        </div>
        ...
        <div class="col30" style="width: 25%;">
            <div class="internalpadding">
				...
				<div class="box">
					...
					{if $addonsavailable}<p><a class="btn btn-primary" href="cart.php?gid=addons&pid={$id}">{$LANG.orderavailableaddons}</a></p>{/if}
				</div>
            </div>
        </div>
        <div class="col70 product-details">
                <table class="table table-striped table-framed table-centered no-more-tables">
                    ...
                    {foreach key=num item=addon from=$addons}
                        <tr>
                            <td data-title="{$LANG.clientareaaddon}">{$addon.name}</td>
                            <td data-title="{$LANG.clientareaaddonpricing}" class="textcenter">{$addon.pricing}</td>
                            <td data-title="{$LANG.clientareahostingnextduedate}" class="textcenter">{$addon.nextduedate}</td>
                            <td data-title="{$LANG.clientareastatus}" class="textcenter"><span class="label {$addon.rawstatus}">{$addon.status}</span></td>
                        </tr>
                    {foreachelse}
                        <tr>

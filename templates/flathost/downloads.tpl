...
<div class="searchbox col-sm-3">
    <form method="post" action="downloads.php?action=search">
        <div class="input-group">
            <input class="form-control" type="text" name="search" value="{$LANG.downloadssearch}" class="input-medium appendedInputButton" onfocus="if(this.value=='{$LANG.downloadssearch}')this.value=''" /><span class="input-group-btn"><button type="submit" class="btn btn-warning">{$LANG.search}</button></span>
        </div>
    </form>
</div>

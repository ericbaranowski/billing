  ...
  <div class="container">
   <div class="row">
    <div class="col-md-4 col-md-offset-4 logo-page"> 
     <a href="index.php" title="{$companyname}"><span class="logo"><span aria-hidden="true" class="icon icon"></span>WHMCS</span></a>
   </div>
 </div>
 {else}
 {$headeroutput}
  ...
  <div class="row">
    <nav class="navbar" role="navigation">
      <div class="navbar-header">
        <div class="navbar-brand"><a href="index.php"><span class="logo"><span aria-hidden="true" class="icon icon"></span>WHMCS</span></a><i class="fa fa-bars fa-lg btn-nav-toggle-responsive"></i></div>
      </div>
      {if $loggedin}
      <div class="collapse navbar-collapse">
        <ul class="nav navbar-nav user-menu navbar-right" id="user-menu">
          <li><a href="#" class="user dropdown-toggle" data-toggle="dropdown"><span class="username"><img src="https://www.gravatar.com/avatar/{php}$userid = $this->_tpl_vars['clientsdetails']['userid'];$result = mysql_query("SELECT email FROM tblclients WHERE id=$userid");$data = mysql_fetch_array($result);$email = $data["email"];echo md5( strtolower( trim( $email ) ) );{/php}?s=50&d=mm" class="user-avatar" alt="">  {$clientsdetails.firstname} {$clientsdetails.lastname} </span></a>

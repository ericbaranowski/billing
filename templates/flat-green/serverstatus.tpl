...
{if $issue.clientaffected}<div class="alert-message block-message alert-warning">{/if}
    ...
    <h3 class="issueh3">{$issue.title} ({$issue.status})</h3>
...
{foreach from=$servers key=num item=server}
        <tr>
            <td data-title="{$LANG.servername}">{$server.name}</td>
            <td data-title="HTTP" class="textcenter" id="port80_{$num}"><img src="images/loadingsml.gif" alt="{$LANG.loading}" /></td>
            <td data-title="FTP" class="textcenter" id="port21_{$num}"><img src="images/loadingsml.gif" alt="{$LANG.loading}" /></td>
            <td data-title="POP3" class="textcenter" id="port110_{$num}"><img src="images/loadingsml.gif" alt="{$LANG.loading}" /></td>
            <td data-title="{$LANG.serverstatusphpinfo}" class="textcenter"><a href="{$server.phpinfourl}" target="_blank">{$LANG.serverstatusphpinfo}</a></td>
            <td data-title="{$LANG.serverstatusserverload}" class="textcenter" id="load{$num}"><img src="images/loadingsml.gif" alt="{$LANG.loading}" /></td>
            <td data-title="{$LANG.serverstatusuptime}" class="textcenter" id="uptime{$num}"><img src="images/loadingsml.gif" alt="{$LANG.loading}" /><script> checkPort({$num},80); checkPort({$num},21); checkPort({$num},110); getStats({$num}); </script></td>
        </tr>
{foreachelse}
        <tr>
...
<p class="textcenter"><img src="images/rssfeed.gif" alt="RSS" align="absmiddle" /> <a href="networkissuesrss.php">{$LANG.announcementsrss}</a></p>
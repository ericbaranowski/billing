...
<tr><td class="radiofield"><input type="checkbox" name="addons[{$addon.id}]" id="a{$addon.id}"{if $addon.status} checked{/if} onclick="recalctotals()" /></td><td class="fieldarea"><label for="a{$addon.id}"><strong>{$addon.name}</strong> - {$addon.pricing}</label><br />{$addon.description}</td></tr>
{/foreach}
</table>
</div>
{/if}
...
<input type="button" value="{$LANG.checkout} &raquo;" class="btn btn-success" onclick="addtocart();" /><br />
<input type="button" value="{$LANG.continueshopping}" class="btn" onclick="addtocart('{$productinfo.gid}');" /><br />
<input type="button" value="{$LANG.viewcart}" class="btn" onclick="window.location='cart.php?a=view'" />
</div>

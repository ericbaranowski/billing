...
<div class="cartbuttons"><input type="button" value="{$LANG.emptycart}" onclick="emptyCart();return false" class="btn" />&nbsp; <input type="button" value="{$LANG.continueshopping}" onclick="window.location='cart.php'" class="btn" /></div>
...
<table width="100%" cellspacing="0" cellpadding="0" class="configtable">
<tr><td class="fieldlabel field1a">{$LANG.clientareafirstname}</td><td class="fieldarea field1b">{if $loggedin}{$clientsdetails.firstname}{else}<input type="text" name="firstname" tabindex="1" style="width:80%;" value="{$clientsdetails.firstname}" />{/if}</td><td class="fieldlabel">{$LANG.clientareaaddress1}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.address1}{else}<input type="text" name="address1" tabindex="7" style="width:80%;" value="{$clientsdetails.address1}" />{/if}</td></tr>
<tr><td class="fieldlabel field2a">{$LANG.clientarealastname}</td><td class="fieldarea field2b">{if $loggedin}{$clientsdetails.lastname}{else}<input type="text" name="lastname" tabindex="2" style="width:80%;" value="{$clientsdetails.lastname}" />{/if}</td><td class="fieldlabel">{$LANG.clientareaaddress2}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.address2}{else}<input type="text" name="address2" tabindex="8" style="width:80%;" value="{$clientsdetails.address2}" />{/if}</td></tr>
<tr><td class="fieldlabel field3a">{$LANG.clientareacompanyname}</td><td class="fieldarea field3b">{if $loggedin}{$clientsdetails.companyname}{else}<input type="text" name="companyname" tabindex="3" style="width:80%;" value="{$clientsdetails.companyname}" />{/if}</td><td class="fieldlabel">{$LANG.clientareacity}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.city}{else}<input type="text" name="city" tabindex="9" style="width:80%;" value="{$clientsdetails.city}" />{/if}</td></tr>
<tr><td class="fieldlabel field4a">{$LANG.clientareaemail}</td><td class="fieldarea field4b">{if $loggedin}{$clientsdetails.email}{else}<input type="text" name="email" tabindex="4" style="width:90%;" value="{$clientsdetails.email}" />{/if}</td><td class="fieldlabel">{$LANG.clientareastate}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.state}{else}<input type="text" name="state" tabindex="10" style="width:80%;" value="{$clientsdetails.state}" />{/if}</td></tr>
<tr>{if !$loggedin}<td class="fieldlabel field5a">{$LANG.clientareapassword}</td><td class="fieldarea field5b"><input type="password" name="password" tabindex="5" id="newpw" size="20" value="{$password}" /></td>{else}<td class="fieldlabel fieldhide"></td><td class="fieldarea fieldhide"></td>{/if}<td class="fieldlabel">{$LANG.clientareapostcode}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.postcode}{else}<input type="text" name="postcode" tabindex="11" size="15" value="{$clientsdetails.postcode}" />{/if}</td></tr>
<tr>{if !$loggedin}<td class="fieldlabel field6a">{$LANG.clientareaconfirmpassword}</td><td class="fieldarea field6b"><input type="password" name="password2" tabindex="6" size="20" value="{$password2}" /></td>{else}<td class="fieldlabel fieldhide"></td><td class="fieldarea fieldhide"></td>{/if}<td class="fieldlabel">{$LANG.clientareacountry}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.country}{else}{$clientcountrydropdown|replace:'<select':'<select tabindex="12"'}{/if}</td></tr>
<tr><td colspan="2" class="fieldarea fieldhide">{if !$loggedin}<script language="javascript">showStrengthBar();</script>{/if}</td><td class="fieldlabel">{$LANG.clientareaphonenumber}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.phonenumber}{else}<input type="text" name="phonenumber" tabindex="13" size="20" value="{$clientsdetails.phonenumber}" />{/if}</td></tr>
...
<div align="center"><input type="submit" value="{$LANG.completeorder}"{if $cartitems==0} disabled{/if} onclick="this.value='{$LANG.pleasewait}'" class="btn btn-success" /></div>

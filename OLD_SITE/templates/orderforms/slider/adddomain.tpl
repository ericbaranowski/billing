...
<a href="cart.php?a=add&domain={$domain}&currency={$curr.id}"><img src="{$BASE_PATH_IMG}/flags/{if $curr.code eq "AUD"}au{elseif $curr.code eq "CAD"}ca{elseif $curr.code eq "EUR"}eu{elseif $curr.code eq "GBP"}gb{elseif $curr.code eq "INR"}in{elseif $curr.code eq "JPY"}jp{elseif $curr.code eq "USD"}us{elseif $curr.code eq "ZAR"}za{else}na{/if}.png" border="0" alt="" /> {$curr.code}</a>
{/foreach}
</div>
<div class="clear"></div>
{else}
...
<div id="loading" class="loading"><img src="{$BASE_PATH_IMG}/loading.gif" border="0" alt="{$LANG.loading}" /></div>

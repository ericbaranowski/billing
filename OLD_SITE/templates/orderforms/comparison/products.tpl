...
<a href="cart.php?gid={$gid}&currency={$curr.id}"><img src="{$BASE_PATH_IMG}/flags/{if $curr.code eq "AUD"}au{elseif $curr.code eq "CAD"}ca{elseif $curr.code eq "EUR"}eu{elseif $curr.code eq "GBP"}gb{elseif $curr.code eq "INR"}in{elseif $curr.code eq "JPY"}jp{elseif $curr.code eq "USD"}us{elseif $curr.code eq "ZAR"}za{else}na{/if}.png" border="0" alt="" /> {$curr.code}</a>
{/foreach}
</div>
<div class="clear"></div>
{/if}
...
{foreach key=num item=product from=$products}
    {**
     * This line will assign a language variable dynamically.
     * This will result in a line like orderpaymenttermmonthly which can be used
     * in further output like {$LANG.$pricingTerm}
     *}
...
<span class="title">{$product.name}</span><br />
    ...
    {else}
        ...
        {/if}
        {$product.pricing.minprice.price} {$LANG.$pricingTerm}
    {/if}<br />
</div>
...
{/foreach}
<div class="{if $num % 2 == 0}a{else}b{/if}feature{if count($products.0.features)}desc{/if}{cycle name=$product.pid values="1,2"}">
<br />

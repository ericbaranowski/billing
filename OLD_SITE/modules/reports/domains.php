...
if (count($incfields)) {
    ...
    foreach ($filterfield as $i => $val) {
        if ($val && array_key_exists($val, $filterfields)) {
            if ($val == 'clientname') {
                $val = "(SELECT CONCAT(firstname,' ',lastname) FROM tblclients WHERE id=tbldomains.userid)";
            }
    ...
    foreach ($incfields AS $fieldname) {
        if (array_key_exists($fieldname,$filterfields)) {
            ...
            if ($fieldname=="clientname") $fieldname = "(SELECT CONCAT(firstname,' ',lastname) FROM tblclients WHERE id=tbldomains.userid)";
            $fieldlist[] = $fieldname;
        }
    }

    $result = select_query("tbldomains", implode(',', $fieldlist), implode(' AND ', $filters));
    while ($data = mysql_fetch_assoc($result)) {
        if (isset($data['registrar'])) $data['registrar'] = ucfirst($data['registrar']);
